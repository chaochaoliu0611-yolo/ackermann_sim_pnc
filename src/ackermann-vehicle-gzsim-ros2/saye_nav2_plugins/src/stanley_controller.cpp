 ...
 * @brief   Stanley 控制器实现 (Nav2 Controller 插件)
 * @details
 *   移植自 my_pnc_core/src/controller.cpp StanleyController。
 *   公式: δ = θ_err + arctan(k·cte / v)
 */
#include "saye_nav2_plugins/stanley_controller.hpp"
    ...
    std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
    ...
    nav2_util::declare_parameter_if_not_declared(node, name + ".stanley_k",  rclcpp::ParameterValue(0.5));
    ...
    stanley_k_  = node->get_parameter(name + ".stanley_k").as_double();
    ...
    RCLCPP_INFO(logger_, "StanleyController configured: k=%.2f", stanley_k_);
}

void StanleyController::setSpeedLimit(const double& sl, const bool& pct) {
    ...
    nav2_core::GoalChecker* goal_checker)
{
    ...
    // ── 1. 前视参考点搜索 (避免最近点弯道拉扯) ──────────────
    double Ld = std::max(0.2, std::min(0.8, v_actual * 1.5));
    ...
    const auto& ref = plan.poses[ref_idx];
    double ryaw = tf2::getYaw(ref.pose.orientation);

    // ── 2. 航向误差 ──────────────────────────────────────────
    double theta_err = normalizeAngle(ryaw - cyaw);

    // ── 3. 横向误差 (用车辆自身航向投影, 更准确) ────────────
    double dx = cx - ref.pose.position.x;
    double dy = cy - ref.pose.position.y;
    double cte = -dx * std::sin(cyaw) + dy * std::cos(cyaw);

    // ── 4. Stanley 公式 ──────────────────────────────────────
    double v_denom = std::max(0.1, v_actual);
    double steer_stanley = theta_err + std::atan2(stanley_k_ * cte, v_denom);
    ...
    double steer = steer_stanley * 0.6 + steer_ff * 0.4;
    steer = std::max(-vp_.max_steer, std::min(vp_.max_steer, steer));

    // ── 5. 低通滤波 ──────────────────────────────────────────
    steer_filtered_ = 0.4 * steer + 0.6 * steer_filtered_;
    ...
    pts.reserve(std::min(15, N - ref_idx));
    for (int i = ref_idx; i < std::min(ref_idx + 15, N); ++i) {
...
PLUGINLIB_EXPORT_CLASS(saye_nav2_plugins::StanleyController, nav2_core::Controller)
